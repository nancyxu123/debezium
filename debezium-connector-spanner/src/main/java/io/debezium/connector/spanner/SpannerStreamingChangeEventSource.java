...
final class SpannerStreamingChangeEventSource
    implements StreamingChangeEventSource<
        SpannerChangeStreamPartition, SpannerChangeStreamOffsetContext> {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(SpannerStreamingChangeEventSource.class);
    ...
    timer.schedule(
        new TimerTask() {
          ...
          public void run() {
            ...
            final Instant watermarkInstant =
                Instant.ofEpochSecond(timestamp.getSeconds(), timestamp.getNanos());
            final Instant now = Instant.now();
            if (timestamp != null && beforeGettingWatermarkMillis % 30 == 0) {
              LOGGER.info(
                  "Time it took to query the low watermark: "
                      + (afterGettingWatermarkMillis - beforeGettingWatermarkMillis));
              LOGGER.info(
                  "The minimum watermark is: "
                      + timestamp.toString()
                      + " which has millis discrepancy: "
                      + (now.toEpochMilli() - watermarkInstant.toEpochMilli()));
            }
          }
        },
        0,
        ...
        while (resultSet.next()) {
          ...
          LOGGER.info(
              "The time it took t retrieve row "
                  + " and the difference in millis for: "
                  + partitionToken
                  + ": "
          ...
          long startQueryMillis =
              Instant.ofEpochSecond(queryStartTime.getSeconds(), queryStartTime.getNanos())
                  .toEpochMilli();
      ...
      while (resultSet.next()) {
        ...
        for (Struct dataChangeRecord : record.getStructList("data_change_record")) {
          ...
          CollectionId collectionId =
              new CollectionId(projectId, instanceId, databaseId, tableName);
          ...
          if ((nowMillis - commitMillis) > 1000) {
            LOGGER.info(
                "The latest commit timestamp: "
                    ...
                    + " and the difference in millis for partition token: "
                    + partitionTokenOrNull
                    + ": "
                    + (nowMillis - commitMillis));
