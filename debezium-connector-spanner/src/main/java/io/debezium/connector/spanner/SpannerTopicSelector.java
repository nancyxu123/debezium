 ...
 */
@ThreadSafe
public class SpannerTopicSelector {

    /**
     * Gets the selector for topics for a given prefix, database name, and collection name. This
     * selector returns "{@code <prefix>.<dbName>.<collectionName>}", and does not use the replica set
     * name.
     *
    ...
    public static TopicSelector<CollectionId> defaultSelector(String prefix, String heartbeatPrefix) {
        return TopicSelector.defaultSelector(
                prefix, heartbeatPrefix, ".", (id, pref, delimiter) -> getTopicName(id, pref, delimiter));
    }

    /**
     * Get the name of the topic for a given prefix, database name, and collection name. This method
     * returns "{@code <prefix><delimiter><dbName><delimiter><collectionName>}", and does not use the
     * replica set name.
     *
     * @param delimiter the string delineating the prefix, database, and collection names; may not be
     *     null
     * @param prefix the prefix; be null or empty if no prefix is needed
     * @return the topic selector; never null
     */
    private static String getTopicName(CollectionId collectionId, String prefix, String delimiter) {
        ...
        sb.add(collectionId.identifier());
